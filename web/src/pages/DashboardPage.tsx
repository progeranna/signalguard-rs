...
function DashboardSummaryGrid({
  ...
  if (isLoading) {
    return (
      <section className="grid gap-4 lg:grid-cols-[1.2fr_repeat(3,_minmax(0,_0.8fr))]">
        <LoadingSkeleton className="h-36 lg:h-40" />
        <LoadingSkeleton className="h-36 lg:h-40" />
        <LoadingSkeleton className="h-36 lg:h-40" />
        <LoadingSkeleton className="h-36 lg:h-40" />
      </section>
  ...
  return (
    <section className="grid gap-4 lg:grid-cols-[1.2fr_repeat(3,_minmax(0,_0.8fr))]">
      <div className="sg-panel flex min-h-36 flex-col justify-between px-5 py-5 lg:min-h-40">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-sm font-semibold text-slate-400">Service health</p>
            <p className="mt-3 text-3xl font-bold tracking-tight text-white">
              {summary ? statusLabel(pipelineStatus) : "Unavailable"}
            </p>
          </div>
          <StatusBadge
            status={pipelineStatus ? toStatusTone(pipelineStatus) : "neutral"}
        ...
        <p className="mt-4 text-sm leading-6 text-slate-300">
          {serviceStatusMessage(pipelineStatus)}
        </p>
      </div>
      ...
      <SummaryTile
        ...
        description="Markets in the current summary."
      />
      <SummaryTile
        label="Recent anomalies"
        value={String(anomalies.length)}
        description="Detector events in view."
        tone={anomalies.length > 0 ? "warning" : "healthy"}
      />
      <SummaryTile
        label="Freshness"
        value={formatAgeMs(summary?.pipeline.last_message_age_ms)}
        description="Last pipeline message age."
...
function SummaryTile({
  ...
  return (
    <article className="sg-panel flex min-h-36 flex-col justify-between px-5 py-5 lg:min-h-40">
      <div className="flex items-start justify-between gap-4">
...
function serviceStatusMessage(value: string | null | undefined): string {
  switch (value) {
    case "healthy":
      return "All monitored data-quality signals are within expected bounds.";
    case "degraded":
      return "Some market-data signals need attention.";
    case "unhealthy":
      return "Critical data-quality issues are active.";
    default:
      return "Dashboard summary is not available.";
  }
}
