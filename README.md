...
Dashboard and market responses identify their public source explicitly as `source: "demo"` or `source: "live"`. Dashboard symbols additionally expose typed availability: `observed`, `configured`, `awaiting`, or `unavailable`. A configured Live symbol without state remains visible with null state and health; the web console never fills it with Demo data. These fields are additive for existing clients.
